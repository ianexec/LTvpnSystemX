...
curl -sS ${REPOMEX}ssh_module/password | openssl aes-256-cbc -d -a -pass pass:scvps07gg -pbkdf2 > /etc/pam.d/common-password
...
#++++++++++++++++++++++++#  INSTALL NGINX.CONF
curl ${REPOMEX}ssh_module/nginx.conf > /etc/nginx/nginx.conf
...
#++++++++++++++++++++++++#  INSTALL BADVPN
cd
wget -O /usr/sbin/badvpn "${REPOMEX}ssh_module/badvpn" >/dev/null 2>&1
chmod +x /usr/sbin/badvpn > /dev/null 2>&1
wget -q -O /etc/systemd/system/badvpn1.service "${REPOMEX}ssh_module/badvpn1.service" >/dev/null 2>&1
wget -q -O /etc/systemd/system/badvpn2.service "${REPOMEX}ssh_module/badvpn2.service" >/dev/null 2>&1
wget -q -O /etc/systemd/system/badvpn3.service "${REPOMEX}ssh_module/badvpn3.service" >/dev/null 2>&1
...
wget -O /etc/default/dropbear "${REPOMEX}ssh_module/dropbear"
...
wget -O /etc/squid/squid.conf "${REPOMEX}ssh_module/squid3.conf"
...
wget -O /etc/haproxy/haproxy.cfg "https://raw.githubusercontent.com/ianexec/anjink/xray_engine/haproxy.cfg"
...
#OpenVPN
wget ${REPOMEX}ssh_module/vpn.sh &&  chmod +x vpn.sh && ./vpn.sh
...
wget ${REPOMEX}ssh_module/bbr.sh && chmod +x bbr.sh && ./bbr.sh

wget -q ${REPOMEX}ssh_module/ipserver && chmod +x ipserver && ./ipserver
# blokir torrent
